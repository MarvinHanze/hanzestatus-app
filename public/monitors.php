...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'create') {
        ...
        if (mb_strlen($name) < 2) {
            ...
            db()->prepare('INSERT INTO hst_monitors (workspace_id, name, url, type, check_interval_seconds, position) VALUES (?, ?, ?, ?, ?, ?)')
                ->execute([$wsId, $name, $url, $type, $interval, $maxPos + 1]);
            $newMonitorId = (int) db()->lastInsertId();
            // Meteen een eerste gesimuleerde status voor vandaag zetten, zodat de monitor direct een status toont.
            refreshMonitorSimulation($wsId);
            auditLog($wsId, (int) $user['id'], 'monitor.create', 'monitor', $newMonitorId, "Monitor aangemaakt: $name");
            $success = 'Monitor aangemaakt.';
        }
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.25rem;">
    <p style="color:var(--hs-text-muted);font-size:.85rem;margin:0;"><?= count($monitors) ?> monitor(s). Statussen worden gesimuleerd — zie de toelichting op de <a href="settings.php" style="color:var(--hs-primary);">instellingenpagina</a>.</p>
    <?php if ($canManage): ?>
...
<div class="hs-modal-backdrop" id="hsCreateMonitorModal">
    <div class="hs-modal" role="dialog" aria-modal="true" aria-labelledby="hsCreateMonitorTitle">
        ...
        <form method="post">
            ...
            <div class="hs-field"><label for="name">Naam</label><input type="text" id="name" name="name" required maxlength="150" placeholder="bv. API Gateway"></div>
            <div class="hs-field"><label for="url">URL</label><input type="url" id="url" name="url" required placeholder="https://api.mijnbedrijf.nl"></div>
            <div class="hs-field">
            ...
            <div class="hs-field">
                ...
                <p class="hs-hint">Alleen weergave in deze demo — er draait geen echte achtergrond-worker; status wordt gesimuleerd per dag.</p>
            </div>
